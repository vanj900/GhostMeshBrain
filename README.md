...
completes 10 000 ticks in a **single life**, reaches the `evolved` stage, and
maintains healthy vitals throughout.  Death remains real at extreme load (3×)
or under sustained stressors — see [Mortality & balance](#mortality--balance)
below.

**Vital signs trajectory, affect signal, free-energy curve, action distribution, and personality-mask pie chart:**

![GhostMesh balanced run — vitals, affect, free energy, masks](docs/run_analysis_balanced.png)

### Balanced 10 000-tick results summary

| Metric | Before tuning | After tuning |
|---|---|---|
| **Lives / Deaths** | 8 / 7 | **1 / 0** |
| **Final stage** | aware | **evolved** |
| **Final health** | 59.3 | **90.7** |
| **Near-death ticks** | 26 | **0** |
| **Avg energy** | 43.0 | **57.5** |
| **Avg heat** | 54.4 | **8.5** |
| **Avg waste** | 11.5 | **4.8** |
| **Avg allostatic load** | 71.63 | **63.21** |
| **Avg affect** | −0.159 | **−0.093** |
| **Avg free energy** | 21.88 | **8.12** |
| **Avg health score** | 67.6 | **82.7** |
| **Max DECIDE streak** | 269 | **272** |
...
**Personality mask distribution (balanced):**

| Mask | Count | % |
|---|---|---|
| DefaultMode | 2 481 | 24.8 % |
| CentralExec | 2 341 | 23.4 % |
| Guardian | 2 037 | 20.4 % |
| Judge | 2 001 | 20.0 % |
| Dreamer | 1 140 | **11.4 %** |
...
> mv docs/run_analysis.png docs/run_analysis_balanced.png
> ```
...
load or sustained stressor pressure it dies honestly.

### What was tuned (v2 — thermal balance)

Four constant changes in `thermodynamic_agency/core/metabolic.py`:

| Constant | Before | After | Effect |
|---|---|---|---|
| `_PASSIVE_COOL_RATE` | 0.05 | **0.20** | Baseline thermal radiation prevents slow heat runaway at normal waste/load |
| `REPAIR_ALLOSTATIC_THRESHOLD` | 55.0 | **70.0** | Avoids repair-zombie: anticipatory REPAIR only fires under genuine cumulative stress |
| `_AL_BETA_REPAIR` | 0.12 | **0.22** | REPAIR action drains allostatic load faster, enabling escape from the AL↔heat loop |
| `_AL_BETA_REST` | 0.08 | **0.14** | REST is a real pressure valve, not a token gesture |

Plus two new mechanisms:
- **Adaptive thermal throttle** — when heat > 65, `compute_load` is scaled to
  80%, forcing self-regulation rather than blind grinding.
- **Anticipatory REST at AL > 65** — REST (via Janitor: −35 heat, −45 waste)
  fires before the higher-threshold REPAIR, directly breaking the AL↔heat
  positive-feedback loop.  REPAIR still fires for genuine structural damage.

What was *not* changed: death thresholds, the waste→heat non-linear cascade
(responsible for death at extreme waste), or energy cost of actions.  The
organism still dies; it just has the tools to manage a normal life.

---

## In Action — 10 000-tick run (pre-tuning, for comparison)

Pre-tuning results at `seed=42`, `compute_load=1.0` — 8 lives / 7 thermal deaths:

![GhostMesh pre-tuning run analysis](docs/run_analysis_10k.png)
...
The plots below are from the earlier 2 000-tick reference simulation
(`seed=42`, `compute_load=1.0`).
